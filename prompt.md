...
| | |
|---|---|
| **This repo** | https://github.com/kvnloo/sol-pi-omp |
| Clone | `git clone https://github.com/kvnloo/sol-pi-omp.git` |
...
1. Clone or open https://github.com/kvnloo/sol-pi-omp. `cd` into that tree. `git fetch origin`. `python3 .verified-oss-loop/rollout.py show`. Branch from `origin/$(python3 .verified-oss-loop/rollout.py get worker_base)` unless the issue names another base. Day-pass PRs target `feature_target`. Overnight PRs target `overnight_target`.
...
```bash
gh issue list --repo kvnloo/sol-pi-omp --label claimable --state open
gh pr list --repo kvnloo/sol-pi-omp --state open
```
...
8. Open a PR on https://github.com/kvnloo/sol-pi-omp at `feature_target` (day) or `overnight_target` (overnight) with `.github/PULL_REQUEST_TEMPLATE.md` filled. Bind evidence to `head_revision`. **Do not merge `main` or `dev`.**
